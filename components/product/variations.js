          ...
          _createClass(_class2, [
              ...
              value: function setupItems() {
                ...
                this.$element
                  ...
                  .each(function (i, element) {
                    var selected = "";
                    var $selected_variation_item = $(element)
                      .parent()
                      .prev()
                      .find(".woo-selected-variation-item-name");
                    var select = $(element)
                      .parent()
                      .find("select.woo-variation-raw-select");
                    var options = select.find("option");
                    var disabled = select.find("option:disabled");
                    var out_of_stock = select.find(
                      "option.enabled.out-of-stock"
                    );
                    ...
                    options.each(function () {
                      if ($(this).val() !== "") {
                        ...
                        selected =
                          current.length === 0 ? eq.val() : current.val();
                      }
                    }); // Disabled
            ...
            },
            {
              ...
              value: function setupItem(
                ...
                // Mark Selected
                $(element)
                  .find("li.variable-item")
                  .each(function (index, el) {
                    var attribute_value = $(el).attr("data-value");
                    var attribute_title = $(el).attr("data-title"); // Resetting LI

                    $(el)
                      .removeClass("selected disabled no-stock")
                      .addClass("disabled");
                    $(el).attr("aria-checked", "false");
                    $(el).attr("tabindex", "-1");
                    $(el).attr("data-wvstooltip-out-of-stock", "");
                    $(el)
                      .find("input.variable-item-radio-input:radio")
                      .prop("disabled", true)
                      .prop("checked", false); // To Prevent blink

                    if (
                      selected.length < 1 &&
                      woo_variation_swatches_options.show_variation_label
                    ) {
                    ...
                    if (_this3.isAjaxVariation()) {
                      ...
                      if (attribute_value === selected) {
                        ...
                        if (
                          ...
                          $selected_variation_item.text(
                            ""
                              .concat(
                                woo_variation_swatches_options.variation_label_separator,
                                " "
                              )
                              .concat(attribute_title)
                          );
                        }

                        $(el).trigger("wvs-item-updated", [
                          selected,
                          attribute_value,
                        ]);
                      }
                      ...
                      if (
                        _.includes(out_of_stocks, attribute_value) &&
                        woo_variation_swatches_options.clickable_out_of_stock
                      ) {
                        $(el).removeClass("disabled").addClass("no-stock");
                        $(el).attr(
                          "data-wvstooltip-out-of-stock",
                          woo_variation_swatches_options.out_of_stock_tooltip_text
                        );
                      }
                    }
                  });
              ...
              value: function setupEvents() {
                ...
                this.$element
                  ...
                  .each(function (i, element) {
                    var select = $(element)
                      .parent()
                      .find("select.woo-variation-raw-select"); // Trigger Select event based on list

                    if (woo_variation_swatches_options.clear_on_reselect) {
                      // Non Selected Item Should Select
                      $(element).on(
                        "click.wvs",
                        "li.variable-item:not(.selected):not(.radio-variable-item)",
                        function (event) {
                          ...
                          $(this).trigger("wvs-selected-item", [
                            value,
                            select,
                            self.$element,
                          ]); // Custom Event for li
                      ...
                      $(element).on(
                        "click.wvs",
                        "li.variable-item.selected:not(.radio-variable-item)",
                        function (event) {
                          event.preventDefault();
                          event.stopPropagation();
                          var value = $(this).val();
                          ...
                          $(this).trigger("wvs-unselected-item", [
                            value,
                            select,
                            self.$element,
                          ]); // Custom Event for li
                      ...
                      $(element).on(
                        "click.wvs",
                        "input.variable-item-radio-input:radio",
                        function (event) {
                          event.stopPropagation();
                ...
                this.$element.on(
                  ...
                  function (event) {
                    ...
                    $(this)
                      .parent()
                      .removeClass(
                        "enabled-display-limit-mode enabled-catalog-display-limit-mode"
                      );
                    $(this).remove();
                  }
                );
            ...
            },
            {
              ...
              value: function update() {
                ...
                this.$element.on(
                  "woocommerce_variation_has_changed.wvs",
                  function (event) {
            ...
            },
            {
              ...
              value: function setUpStockInfo() {
                ...
                if (woo_variation_swatches_options.show_variation_stock) {
                  var max_stock_label = parseInt(
                    woo_variation_swatches_options.stock_label_threshold,
                    10
                  );
                  this.$element.on("wvs-selected-item.wvs", function (event) {
                    var attributes = _this5.getChosenAttributes();

                    var variations = _this5.findStockVariations(
                      _this5.product_variations,
                      attributes
                    );
                    ...
                    if (
                      attributes.count > 1 &&
                      attributes.count === attributes.mayChosenCount
                    ) {
                      variations.forEach(function (data) {
                        var stockInfoSelector = '[data-attribute_name="'
                          .concat(data.attribute_name, '"] > [data-value="')
                          .concat(data.attribute_value, '"]');

                        if (
                          data.variation.is_in_stock &&
                          data.variation.max_qty &&
                          data.variation.variation_stock_left &&
                          data.variation.max_qty <= max_stock_label
                        ) {
                          _this5.$element
                            .find(
                              "".concat(
                          ...
                          _this5.$element
                            .find(stockInfoSelector)
                            .addClass("wvs-show-stock-left-info");
                        } else {
                          _this5.$element
                            .find(stockInfoSelector)
                            .removeClass("wvs-show-stock-left-info");
            ...
            },
            {
              ...
              value: function findStockVariations(
                allVariations,
                selectedAttributes
              ) {
                var found = [];
                ...
                for (
                  var _i = 0,
                    _Object$entries = Object.entries(selectedAttributes.data);
                  ...
                  if (attribute_value.length === 0) {
                    ...
                    try {
                      for (_iterator.s(); !(_step = _iterator.n()).done; ) {
                        ...
                        var compare = _.extend(
                          selectedAttributes.data,
                          _defineProperty({}, attribute_name, value)
                        );

                        var matched_variation = this.findMatchingVariations(
                          allVariations,
                          compare
                        );
    ...
    jQuery(function ($) {
      try {
        $(document).on("woo_variation_swatches_init", function () {
          ...
          $(
            ".woo_variation_swatches_variations_form:not(.wvs-loaded)"
          ).WooVariationSwatches(); // Yith Composite Product

          $(
            ".ywcp_inner_selected_container:not(.wvs-loaded)"
